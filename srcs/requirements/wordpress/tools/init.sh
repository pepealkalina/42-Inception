...
if ! [[ -f /var/www/wordpress/wp-config.php ]]; then
    ...
    wp core install --allow-root --url=$URL --title=$MARIA_DB_USER --admin_user=$MARIA_DB_USER --admin_password=$MARIA_DB_PASS --admin_email=$WP_EMAIL --path=/var/www/wordpress
    wp user create $WP_USER $WP_EMAIL2 --user_pass=$WP_PASS --role=author --allow-root --path=/var/www/wordpress
else
